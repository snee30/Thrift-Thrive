...
const Content = () => {
  ...
  return (
    <footer className="p-10 bg-[var(--sage)] relative">
      {/* Popup Modal */}
      {activePopup && (
        <div className="fixed inset-0 backdrop-blur-sm flex items-center justify-center z-[1000]">
          {" "}
          {/* Increased z-index */}
          <div className="bg-[var(--cream)] p-6 rounded-lg shadow-2xl max-w-md w-full max-h-[80vh] overflow-y-auto z-[1001]">
            {" "}
            {/* Even higher z-index for content */}
            {popupContents[activePopup]}
            <button
              onClick={() => setActivePopup(null)}
              className="mt-4 bg-[var(--brown)] w-full p-2 text-[var(--cream)] rounded-lg hover:bg-[var(--darkbrown)] cursor-pointer"
            >
              Close
            </button>
          </div>
